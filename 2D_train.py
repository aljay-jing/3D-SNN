...
class Config():
    ...
    def create_data_couple():
        ...
        data_couple = []
        # 正样本对数据集
        same_path = glob.glob(Config.training_dir + '/same_train/*.png')
...
class SiameseNetwork(fluid.dygraph.Layer):
    ...
    def __init__(self):
        ...
        self.linear1 = nn.Linear(input_dim=3 * 12 * 12, output_dim=3 * 12 * 6)
        self.linear2 = nn.Linear(input_dim=3 * 12 * 6, output_dim=3 * 12 * 3)
        self.linear3 = nn.Linear(input_dim=3 * 12 * 3, output_dim=54)
