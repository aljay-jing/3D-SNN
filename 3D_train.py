...
class Config():
    training_dir = "D:/SiameseNetwork/dataset/3D"
    train_batch_size = 1
    ...
    def create_data_couple():
        ...
        data_couple = []
        # # 正样本对数据集
        same_path = glob.glob(Config.training_dir + '/same_train/*')
...
class SiameseNetwork(fluid.dygraph.Layer):
    ...
    def __init__(self):
        ...
        self.linear1 = nn.Linear(input_dim=71 * 12 * 12, output_dim=71 * 12 * 6)
        self.linear2 = nn.Linear(input_dim=71 * 12 * 6, output_dim=71 * 12 * 3)
        self.linear3 = nn.Linear(input_dim=71 * 12 * 3, output_dim=1278)
...
class ContrastiveLoss(Layer):
    ...
    def forward(self, output1, output2, label):
        ...
        loss_contrastive = layers.elementwise_mul(1 - label, layers.square(euclidean_distance),
            ...
            paddle.clip(self.margin - euclidean_distance, min=0.0)), axis=0)
...
if __name__ == "__main__":
    ...
    with fluid.dygraph.guard(place):
        ...
        os.makedirs('D:/SiameseNetwork/figure_and_csv', exist_ok=True)
        # train_cost.plot(
